...
void Criptare::Vigenere(const char* in, const char* out, const char* key)
{
	int n = strlen(key);
	Matrix B(n, 1);
	for (int i = 0; i < n; i++)
	{
		B[i][0] = Alphabet::ToCode(key[i]);
	}
	...
	while (!done)
	{
		...
		x[0][0] = Alphabet::ToCode(c);

		int k = 1;
		while (k < n)
		{
			x[k++][0] = (input >> std::noskipws >> c) ? Alphabet::ToCode(c) : 0;
		}
		...
		for (int i = 0; i < n; i++)
		{
			output << Alphabet::ToChar(result[i][0]);
		}
	}
